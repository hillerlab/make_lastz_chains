...
def psl_bundle(cat_out_dirname, project_paths, executables, params):
    # 1.1 -> sort
    concatenated_files = [
        os.path.join(cat_out_dirname, x) for x in os.listdir(cat_out_dirname)
    ]
...
def make_chains_joblist(
    ...
    matrix = (
        params.lastz_q if hasattr(params, "lastz_q") else ""
    )  # add lastz_q if it exists
    ...
    for bundle_filename in bundle_filenames:
        ...
        out_path = os.path.join(
            project_paths.chain_output_dir, f"{bundle_filename}.chain"
        )
        cmd = [
            executables.axt_chain,
            "-psl",
            "-verbose=0",
...
def do_chain_run(
    ...
    to_log(
        f"Saving {len(chain_jobs)} axtChain jobs to {project_paths.chain_joblist_path}"
    )
