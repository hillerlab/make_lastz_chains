...
- v1 path: cache whole-chromosome FASTA extractions per task. `extract_chrom_to_fasta` now writes to a deterministic path `./_v1_chrom_cache/<basename.2bit>_<chrom>.fa` in the Nextflow work directory and reuses on cache hit. Without this, BULK partitions re-extracted the (shared) query chromosome from the v1 `.2bit` once per BULK iteration — up to 100× of redundant `twoBitToFa` work per task — pushing tasks past the 30-min `process_fast` time limit on large genomes
